...
I1_big=uint8(zeros(row+6,col+6,3));

for k=1:3
    for r=1:row
        for c=1:col
            I1_big(r+3,c+3,k)=I1(r,c,k);
        end
    end
end

for i=1:size(I1)
     I1_big(1:3,i) = I1_big(4,i);
 end
 
 for i=1:size(I1)
     I1_big(row+1:row+3,i) = I1_big(row,i);
 end 

 for i=1:size(I1)
     I1_big(i,1:4) = I1_big(i,4);
 end
 
 for i=1:size(I1)
     I1_big(i,col+1:col+3) = I1_big(i,col);
 end

for k=1:3
    for r= 1:size(I1_big,1)-6
        for c=1:size(I1_big,2)-6
            window=zeros(1,49,k); 
            count=1;
            for i=1:7
                for j=1:7
...
fid = fopen('Problem2b_barn7_512.raw','w+');
